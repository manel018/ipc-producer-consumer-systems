...
def plot_execution_times():
    ...
    config_labels = ['P1C8', 'P1C4', 'P1C2', 'P1C1', 'P2C1', 'P4C1', 'P8C1']
    ...
    # Setas de tendência no fundo: mais consumidores (esquerda) e mais produtores (direita).
    center_x = (len(config_labels) - 1) / 2
    left_end = -0.15
    right_end = len(config_labels) - 0.55
    arrow_y = 0.52
    text_y = 0.55
    data_x_axes_y = mtransforms.blended_transform_factory(ax.transData, ax.transAxes)

    ax.annotate(
        '',
        xy=(left_end, arrow_y),
        xytext=(center_x - 0.6, arrow_y),
        xycoords=data_x_axes_y,
        textcoords=data_x_axes_y,
        arrowprops=dict(arrowstyle='->', color='firebrick', lw=8.0, alpha=0.4, mutation_scale=50),
        zorder=1,
    )
    ax.annotate(
        '',
        xy=(right_end, arrow_y),
        xytext=(center_x + 0.2, arrow_y),
        xycoords=data_x_axes_y,
        textcoords=data_x_axes_y,
        arrowprops=dict(arrowstyle='->', color='firebrick', lw=8.0, alpha=0.4, mutation_scale=50),
        zorder=1,
    ...
    left_text.set_path_effects([
        mpe.Stroke(linewidth=2.6, foreground='firebrick'),
        mpe.Normal(),
    ])

    right_text = ax.text(
        (right_end + center_x) / 2,
        text_y,
...
def plot_buffer_occupancy_improved():
    """Gráficos de ocupação para cada N"""
    ...
    for N, files in sorted(files_by_N.items()):
        ...
        # Ordenar arquivos na mesma sequência térmica usada nas demais funções
        ordered_files = sorted(
            files,
            key=lambda name: (
                ordered_configs.index(
                    (
